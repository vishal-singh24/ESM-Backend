...
exports.downloadKmz = async (req, res) => {
  try {
    ...
    if (!feederName) {
      // Debug information
      console.log("GPS Details structure:", userWaypoints[0]?.gpsDetails);
      return res.status(400).json({
    ...
    console.log("Generating KMZ for route type:", routeType);
    const kmzBuffer = await generateKmzBuffer(userWaypoints, routeType);

    // Send KMZ file with feederName only
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=${sanitizedFeederName}_feeder_${empId}_route.kmz`
    );
